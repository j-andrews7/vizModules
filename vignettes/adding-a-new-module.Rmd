...
- [ ] If the module's output is not a plotly graph, attach a `vector_svg` attribute to the reactive its server returns so its panels appear in the Figure Builder's SVG export; `.draw_to_svg()` builds one from any grid or base drawing. See `ComplexHeatmap_HeatmapServer()`.
